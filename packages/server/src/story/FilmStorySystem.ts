...
export class FilmStorySystem {
  ...
  command(agent: AgentState, target: string, tick: number): string {
    ...
    if (!this.near(agent, step)) return state.scene === 'm1_mirror' && state.step === 0 ? '走到裂镜前，伸手可及后再按 G。' : '请走近金色目标标记（4 米内），再按 G。';
