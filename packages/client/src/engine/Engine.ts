...
import { insideLifeRoom, meetingLocked, meetingCarPose, interrogationLocked, pillLocked, lafayetteKnocking, lafayetteWelcomeLocked, awakeningLocked, oracleActing, phoneLocked, heldPhone, wakeCallLocked, sentinelLocked, interludeLocked, rescueLocked, lobbyLocked, governmentLocked, airRescueLocked, matrixEscapeLocked, theOneLocked, helElevatorLocked, helDanceDoorLocked, rescueLoadout, windowOpening, windowCrossing, OFFICE_CONTACT, LOBBY_ENTRY, GOVERNMENT_RESCUE, FILM_SETS, HEL_COATCHECK } from '@auto_matrix/shared';
...
export class Engine {
  ...
  setSandbox(state: SandboxState, agents: Record<string, AgentState>): void {
    ...
    if (after?.scene === 'm1_sentinels' && !after.visiting && after.actor === this.playerControls?.id && this.running) {
      ...
      if (current && current.phase !== previous?.phase) {
        if (current.phase === 'shutdown') this.audio.sentinelSound('alarm');
        else if (current.phase === 'sweep') this.audio.sentinelSound('powerDown');
        else if (current.phase === 'detected') this.audio.sentinelSound('detected');
    ...
    if (after?.rescue && ['m1_rescue_decision', 'm1_guns'].includes(after.scene) && !after.visiting && after.actor === this.playerControls?.id && this.running) {
      const previous = before?.scene === after.scene ? before.rescue : undefined; const current = after.rescue;
      if (current.phase !== previous?.phase) {
        if (current.phase === 'briefing') { this.audio.rescueSound('hologram'); this.audio.dialogue(); }
        else if (current.phase === 'racks_arriving') this.audio.rescueSound('racks');
    ...
    if (after?.government && ['m1_smith_question', 'm1_bullet_dodge'].includes(after.scene) && !after.visiting && after.actor === this.playerControls?.id && this.running) {
      const previous = before?.scene === after.scene ? before.government : undefined; const current = after.government;
      if (current.phase !== previous?.phase) {
        if (current.phase === 'monologue') { this.audio.governmentSound('earpiece'); this.audio.dialogue(); }
        else if (current.phase === 'alarm') { this.audio.governmentSound('alarm'); this.audio.dialogue(); }
        else if (current.phase === 'opening') this.audio.governmentSound('gunfire');
        else if (current.phase === 'bullet_time') this.audio.governmentSound('bullet');
        else if (current.phase === 'trinity') this.audio.governmentSound('body');
        else if (current.phase === 'downloading') { this.audio.governmentSound('phone'); this.audio.dialogue(); }
      }
      if (previous?.phase === current.phase) {
        if (current.phase === 'monologue' && previous.elapsed < 8.2 && current.elapsed >= 8.2) this.audio.governmentSound('serum');
        if (current.phase === 'bullet_time') for (const beat of GOVERNMENT_RESCUE.rooftop.beats) {
          if (previous.elapsed < beat && current.elapsed >= beat) this.audio.governmentSound('bullet');
        }
        if (current.phase === 'trinity' && previous.elapsed < 1.35 && current.elapsed >= 1.35) this.audio.governmentSound('gunfire');
        if (current.phase === 'downloading' && previous.elapsed < 1.6 && current.elapsed >= 1.6) this.audio.governmentSound('upload');
        if (current.phase === 'downloading' && previous.elapsed < 3.8 && current.elapsed >= 3.8) this.audio.governmentSound('rotor');
      }
    }
    if (after?.airRescue && ['m1_helicopter', 'm1_rooftop_rescue'].includes(after.scene) && !after.visiting && after.actor === this.playerControls?.id && this.running) {
      const previous = before?.scene === after.scene ? before.airRescue : undefined; const current = after.airRescue;
      if (current.phase !== previous?.phase) {
        if (current.phase === 'approach' || current.phase === 'impact') this.audio.governmentSound('rotor');
        else if (current.phase === 'firing') this.audio.governmentSound('minigun');
        else if (current.phase === 'leap_window') this.audio.governmentSound('glass');
        else if (current.phase === 'catching' || current.phase === 'bracing') this.audio.governmentSound('rope');
        else if (current.phase === 'pulling') this.audio.governmentSound('cut');
      }
      if (previous?.phase === current.phase) {
        if ((current.bursts ?? 0) > (previous.bursts ?? 0)) { this.audio.governmentSound('minigun'); this.audio.governmentSound('glass'); }
        if ((current.braces ?? 0) > (previous.braces ?? 0) || (current.misses ?? 0) > (previous.misses ?? 0)) this.audio.governmentSound('rope');
        if (!previous.ropeCut && current.ropeCut) this.audio.governmentSound('cut');
        if (!previous.crash && current.crash) this.audio.governmentSound('crash');
      }
    }
    if (after?.matrixEscape && ['m1_subway', 'm1_city_chase'].includes(after.scene) && !after.visiting && after.actor === this.playerControls?.id && this.running) {
      const previous = before?.scene === after.scene ? before.matrixEscape : undefined; const current = after.matrixEscape;
      if (current.phase !== previous?.phase) {
        if (current.phase === 'phone_shot') this.audio.matrixEscapeSound('phone-shot');
        else if (current.phase === 'stance') this.audio.matrixEscapeSound('stance');
        else if (current.phase === 'wall_break') this.audio.matrixEscapeSound('wall');
        else if (current.phase === 'tracks') this.audio.matrixEscapeSound('tracks');
        else if (current.phase === 'train_window') this.audio.matrixEscapeSound('train');
        else if (current.phase === 'train_escape') this.audio.matrixEscapeSound('escape');
        else if (current.phase === 'body_swap' || current.phase === 'possession') this.audio.matrixEscapeSound('code');
        else if (current.phase === 'briefing') { this.audio.matrixEscapeSound('earpiece'); this.audio.dialogue(); }
        else if (current.phase === 'phone_failure') this.audio.matrixEscapeSound('phone-shot');
        else if (current.phase === 'truck_warning' || current.phase === 'truck_window') this.audio.matrixEscapeSound('truck');
        else if (current.phase === 'door') this.audio.matrixEscapeSound('door');
      }
      if (previous?.phase === current.phase) {
        if (!previous.phoneBroken && current.phoneBroken) this.audio.matrixEscapeSound('phone-shot');
        if (!previous.wallBroken && current.wallBroken) this.audio.matrixEscapeSound('wall');
        if (!previous.trainHit && current.trainHit) this.audio.matrixEscapeSound('train-impact');
    ...
    if (after?.theOne && ['m1_death', 'm1_return', 'm1_final_call'].includes(after.scene) && !after.visiting && after.actor === this.playerControls?.id && this.running) {
      ...
      if (current.phase !== previous?.phase) {
        if (current.phase === 'ambush') this.audio.theOneSound('door');
        else if (current.phase === 'gunfire') this.audio.theOneSound('gunshots');
        else if (current.phase === 'flatline') this.audio.theOneSound('flatline');
        else if (current.phase === 'listening') this.audio.dialogue();
        else if (current.phase === 'kiss') this.audio.theOneSound('kiss');
        else if (current.phase === 'revive') this.audio.theOneSound('heartbeat');
        else if (current.phase === 'code_reveal' || current.phase === 'dive') this.audio.theOneSound('code');
        else if (current.phase === 'bullet_window' || current.phase === 'bullet_stop') this.audio.theOneSound('bullets');
        else if (current.phase === 'burst') this.audio.theOneSound('burst');
        else if (current.phase === 'exit_run') this.audio.theOneSound('alarm');
        else if (current.phase === 'exit_phone') this.audio.theOneSound('call');
        else if (current.phase === 'call') { this.audio.theOneSound('call'); this.audio.dialogue(); }
        else if (current.phase === 'takeoff') this.audio.theOneSound('wind');
      }
    ...
    if (this.playerControls) {
      ...
      this.playerControls.performing = Boolean(journey?.actor === this.playerControls.id && (journey.scene === 'm1_room303' && ['breach', 'dive', 'ladder_ready'].includes(journey.openingHotel?.phase ?? '') || journey.trucks?.phase === 'rescue' || helElevatorLocked(journey) || helDanceDoorLocked(journey) || meetingLocked(journey) || awakeningLocked(journey) || oracleActing(journey) || phoneLocked(journey) || wakeCallLocked(journey) || sentinelLocked(journey) || interludeLocked(journey) || rescueLocked(journey) || lobbyLocked(journey) || governmentLocked(journey) || airRescueLocked(journey) || matrixEscapeLocked(journey) || theOneLocked(journey) || reloadedLocked(journey) || windowOpening(journey) || windowCrossing(journey) || pillLocked(journey) || interrogationLocked(journey) || lafayetteKnocking(journey) || lafayetteWelcomeLocked(journey)));
      this.playerControls.mirror = journey?.actor === this.playerControls.id && !journey.visiting && journey.scene === 'm1_mirror' ? (journey.awakening?.elapsed ?? 0) / 8 : 0;
