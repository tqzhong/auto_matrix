...
export class AgentRenderer {
  ...
  update(delta: number, camera?: THREE.Camera, speed = 1, tick = 0, journey?: FilmJourney): void {
    for (const [id, entry] of this.agents) {
      ...
      entry.shadow.visible = state.status !== 'disconnected' && !state.currentAction?.parameters.filmDuel && !mountainFlying && !catchFlying && !coma && !pod && !epilogueCarried && !input.truckPassenger;
