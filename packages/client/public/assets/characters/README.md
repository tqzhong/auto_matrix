...
GLB files in this directory reference their neighboring PNG textures. Keep the directory together when importing them elsewhere. Only the four final GLBs, their referenced textures and the runtime frontal atlas are retained here, alongside documentation and attribution. Raw skin/eye/suit textures are intermediate files regenerated by the builder. Generated `.blend` projects pack their images internally.
...
`tests/character-asset.test.mts` loads all four shipped GLBs with Three.js. It checks bones, normalized weights, transparent eye surfaces, hand articulation, ground contact and finite mesh deformation through running, landing and punches. Browser inspection covers front/profile views, UV seams, hair, glasses, outfits and motion; these visual properties are not proven by the unit tests.
