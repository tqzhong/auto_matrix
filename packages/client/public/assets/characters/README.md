...
GLB files in this directory reference their neighboring PNG textures. Keep the directory together when importing them elsewhere. The four character GLBs, Neo's office outfit GLB, their referenced textures and the runtime frontal atlas are retained here, alongside documentation and attribution. Other raw skin/eye/suit textures are intermediate files regenerated by the builder. Generated `.blend` projects pack their images internally.
