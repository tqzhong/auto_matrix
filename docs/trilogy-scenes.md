...
最新范围已调整为选取关键电影段落，优先做好可玩性与布景，不要求穷举影片所有镜头。大堂营救已增加专用枪战与布景；后续优先级、验收证据和限制见 [关键片段制作记录](key-scenes.md)。下面的 102 段仍是现有剧情索引。

## 当前完成度

这是可运行的三部曲场景原型：102 个剧情段落、57 个可进入场景、38 类建筑布局。计数按本项目任务粒度划分，不是电影逐镜头统计，也不代表原片所有室内房间、镜头与动作已经一比一复现。当前空间主要由程序生成的几何与 PBR 材质组成，尚未达到电影级美术验收标准。
...
| 102 段剧情、角色、目标、叙述与音乐匹配 | packages/shared/src/constants/film-story.ts |
...
新区域按当前人物位置加载，离开时释放自身几何、材质与纹理；旧城市继续作为日常生活区域。57 个地点均有对应场景实现，但部分地点仍共用建筑家族，不能据此宣称美术已经逐场验收。
...
1. **逐镜头核对**：需要逐一比对成片的时间码、镜头、空间相邻关系、出场人物与道具。目前的 102 段是剧情索引，不是完整镜头清单。
...
5. **体验调优**：102 段间使用显式继续切换，尚未做到同一无缝大地图。基础验收包含全部路线的服务器测试与部分场景实机检查；不能据此宣称已经手动玩完全部路线或确认所有画面达标。
...
| m1_sentinels | 静默的飞船 | 地下隧道 · 舰船撤离 | Morpheus | 交互 → 交互 |
...
| m1_bathroom | 为同伴争取时间 | 旧楼 · 黑猫与伏击 | Morpheus | 战斗 → 交互 |
| m1_unplugged | 背叛发生在现实 | 尼布甲尼撒号 · 核心与医疗舱 | Tank | 移动 → 交互 |
...
- 130 项现有及新增测试全部通过。102 段路线测试直接调用服务端交互与近战逻辑，把测试人物移动到目标位置；不是手动正常游玩录像。
