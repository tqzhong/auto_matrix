...
最新范围已调整为选取关键电影段落，优先做好可玩性与布景，不要求穷举影片所有镜头。大堂营救已增加专用枪战与布景；后续优先级、验收证据和限制见 [关键片段制作记录](key-scenes.md)。下面的 105 段是当前剧情索引。
...
| Logos 防线、云海、太阳与飞行载具 | packages/client/src/engine/LogosFlightRenderer.ts |
...
| m3_oracle_request | 另一边的营救 | 先知公寓 · 候诊室与厨房 | Trinity | 确认先知改貌 → 询问 Neo → 信任反思 → 跟随 Seraph |
...
| m3_neo_carried | 光中的身体 | 机器核心 · Deus Ex Machina | Neo | 按 G → 连接逐条缩回 → 身体放低到发光托架 → 转上机器驳船并离开 |
| m3_dawn | Sati 留下的日出 | 公园 · 新的日出 | The Oracle | 走到长椅按 G → 看重复黑猫与路面重置 → J 回应建筑师的和平承诺 → 走近 Sati 按 G → 看日出 → 明确确认本轮结束 |
...
- 3-12 尾声专项覆盖 Kid 目击退兵/跑动报信、两组重逢、Neo 连接收回/放低/发光托架与驳船转运、黑猫重复/路面重置、建筑师放行承诺、玩家哲学回应、Sati 日出及不自动开启新循环的明确确认；暂停、断线、存档恢复、角色占用、资源释放和第一/第三人称也有回归。隔离页面实际复查 Neo 横卧转运构图与页面日志，原 5174 玩家存档未替换。
...
- 623 项现有及新增测试全部通过。106 段路线测试已实际执行浴室坚守/击退、Tank 反击/重连、两段 Logos 航行、Logos 告别、Smith 终局和尾声等专用机制；其余普通节点仍可能通过测试定位完成，不是手动正常游玩录像。
